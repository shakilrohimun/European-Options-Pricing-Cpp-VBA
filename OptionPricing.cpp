...
double Option::localVolCallPrice(double spot, double strike, double rate, double maturity, size_t time_step, size_t pos_step) {
    const double dt = maturity / time_step, dx = 2 * strike / pos_step;
    std::vector<std::vector<double>> grid(time_step + 1, std::vector<double>(pos_step + 1, 0));

    // Définir la condition de gain terminal pour une option call
    for (int j = 0; j <= pos_step; ++j) {
        grid[time_step][j] = maximum(j * dx - strike, 0.0);
    }

    // Coefficients pour le schéma implicite
    std::vector<double> a(pos_step - 1, 0.0);
    std::vector<double> b(pos_step - 1, 0.0);
    std::vector<double> c(pos_step - 1, 0.0);
    std::vector<double> d(pos_step - 1, 0.0);

    // Méthode des différences finies (Schéma implicite utilisant l'algorithme de Thomas)
    for (int i = time_step - 1; i >= 0; --i) {
        for (int j = 1; j < pos_step; ++j) {
            double S = j * dx;
            ...
            else if (j == pos_step - 1) {  // Conditions aux limites à la frontière supérieure
                a[j - 1] = alpha;
                b[j - 1] = 1.0 - beta;
                c[j - 1] = 0.0;
    ...
    return std::exp(-rate * maturity) * grid[0][pos_step / 2];  // Prix actualisé
}
